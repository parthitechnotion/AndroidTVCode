...
import com.googleandroidtv.animation.ViewDimmer;
import com.googleandroidtv.animation.ViewDimmer.DimState;
import com.googleandroidtv.apps.AppsAdapter;
import com.googleandroidtv.apps.BannerSelectedChangedListener;
...
import com.googleandroidtv.widget.EditModeViewActionListener;
...
public class ActiveItemsRowView extends HorizontalGridView implements OnChildSelectedListener, OnHierarchyChangeListener, BannerSelectedChangedListener, EditModeViewActionListener, OnEditModeChangedListener, OnGlobalFocusChangeListener {
    ...
    private void adjustNumRows() {
        ...
        Log.e("CountApp","-"+getAdapter().getItemCount());
        double row = (double) getAdapter().getItemCount()/20;

        String SRow = String.valueOf(row);
      /*  int raw2= (int) row;
        float raw22 = (float)raw2;
        if(row> raw22)
        {
         Log.e("TRow","-"+raw2++);
        }*/
        if( getAdapter().getItemCount()>20) {
            if (SRow.contains(".")) {
                String[] approw = SRow.split("\\.");
                Log.e("approw", "-" + approw[0]);
                TotalnumRow = Integer.parseInt(approw[0]);
                if(!approw[1].equals("0")) {
                    TotalnumRow++;
                }
            }
        }
        Log.e("CountAppRow","-"+row);
        if (getAdapter().getItemCount() >= res.getInteger(R.integer.two_row_cut_off)) {
            integer = TotalnumRow;/*res.getInteger(R.integer.max_num_banner_rows);*/
        } else {
