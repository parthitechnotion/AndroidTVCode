...
import com.googleandroidtv.HomeScrollManager.HomeScrollFractionListener;
import com.googleandroidtv.R;
import com.googleandroidtv.apps.AppsAdapter;
import com.googleandroidtv.apps.AppsRanker;
import com.googleandroidtv.apps.AppsUpdateListener;
import com.googleandroidtv.apps.ConnectivityListener;
import com.googleandroidtv.apps.ConnectivityListener.Listener;
import com.googleandroidtv.apps.LaunchPointListGenerator;
import com.googleandroidtv.apps.OnEditModeChangedListener;
import com.googleandroidtv.apps.SettingsAdapter;
import com.googleandroidtv.custom_app.CustomAppAdaptor;
import com.googleandroidtv.inputs.InputsAdapter;
...
import com.googleandroidtv.notifications.NotificationsServiceAdapter;
import com.googleandroidtv.notifications.PartnerAdapter;
import com.googleandroidtv.util.Partner;
import com.googleandroidtv.util.Preconditions;
...
public class HomeScreenAdapter extends Adapter<HomeScreenAdapter.HomeViewHolder> implements OnChildSelectedListener, HomeScreenRow.RowChangeListener, Listener, OnEditModeChangedListener {
    ...
    private void buildRowList() {
        ...
        //推荐
      buildRow(8, 1,  res.getString(R.string.home_collection), null, partnerFont, R.dimen.home_scroll_size_apps, false);  // collection
       /* //视频
          buildRow(9, 3,  res.getString(R.string.home_video), this.mPartner.getRowIcon("partner_row"), partnerFont, R.dimen.home_scroll_size_apps, false); // home video
        //音乐
        buildRow(10, 4,  res.getString(R.string.home_music), this.mPartner.getRowIcon("inputs_row"), partnerFont, R.dimen.home_scroll_size_apps, false); // home music
        //所有应用*/
        buildRow(3, 2,  res.getString(R.string.category_label_apps), this.mPartner.getRowIcon("apps_row"), partnerFont, R.dimen.home_scroll_size_apps, false); // app row
        //设置
        buildRow(5, 5, this.mPartner.getRowTitle("settings_row", res.getString(R.string.category_label_settings)), this.mPartner.getRowIcon("settings_row"), partnerFont, R.dimen.home_scroll_size_settings, false); // setting row

        buildRow(15, 4, res.getString(R.string.category_label_store), this.mPartner.getRowIcon("apps_row"), partnerFont, R.dimen.home_scroll_size_apps, true);  // Custom row

        buildRow(35, 3, res.getString(R.string.category_recent_app), this.mPartner.getRowIcon("apps_row"), partnerFont, R.dimen.home_scroll_size_apps, true);  // Recent row
    ...
    private Adapter<?> initAdapter(int type) {
        switch (type) {
            ...
            case android.support.v7.preference.R.styleable.Preference_android_key /*6*/:
                ...
                return adapter;
            case 15:
                return new AppsAdapter(this.mMainActivity, 25, this.mLaunchPointListGenerator, this.mAppsRanker, null);
            case 35:
                return new AppsAdapter(this.mMainActivity, 45, this.mLaunchPointListGenerator, this.mAppsRanker, null);
               /* String json = Utils.inputStreamToString(getResources().openRawResource(R.raw.card_settings));
                CardRow cardRow = new Gson().fromJson(json, CardRow.class);
                Presenter presenter = new IconCardPresenter(getActivity());
                ArrayObjectAdapter listRowAdapter = new ArrayObjectAdapter(presenter);
                return new CustomAppAdaptor(this.mMainActivity,this.mLaunchPointListGenerator);
            return new CustomAppAdaptor(this.mMainActivity,this.mLaunchPointListGenerator);*/
            default:
                return null;
        }
